...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    public void setAnswer(String answer) {
        this.mAnswer = transformation(answer.trim());
        LogUtils.v("GestureFragment",mAnswer.toString());
    }

    private int[] transformation(String answer) {
        int[] answers = new int[answer.trim().length()];
        for (int i = 0; i < answer.trim().length(); i++) {
            answers[i] = Integer.parseInt(String.valueOf(answer.charAt(i)))+1;
        }
        return answers;
