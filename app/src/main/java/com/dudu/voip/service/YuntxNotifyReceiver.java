...
public class YuntxNotifyReceiver extends ECNotifyReceiver {
    ...
    public void onNotificationClicked(Context context, int notifyType, String sender) {
        LogUtils.d("ECSDK_Demo.YuntxNotifyReceiver", "onNotificationClicked notifyType " + notifyType + " ,sender " + sender);
...
//        context.startActivity(intent);
    ...
    public void onSoftVersion(Context context, String version , String softDesc, boolean force) {
    	...
    	VoipSDKCoreHelper.setSoftUpdate(version, softDesc, force);
    }
    ...
    public static class NotifyService extends Service {
        ...
        private void receiveImp(Intent intent) {
            if(intent == null) {
                LogUtils.e(TAG, "receiveImp receiveIntent == null");
                return ;
            }
            LogUtils.i(TAG, "intent:action " + intent.getAction());
            int optCode = intent.getIntExtra(SERVICE_OPT_CODE, 0);
            if(optCode == 0) {
                LogUtils.e(TAG, "receiveImp invalid opcode.");
                return ;
            }
            if(!VoipSDKCoreHelper.isUIShowing()) {
                VoipSDKCoreHelper.init(this);
            }
            switch (optCode) {
                case EVENT_TYPE_CALL:
                    LogUtils.d(TAG, "receive call event ");
...
//                    startActivity(call);
        ...
        public int onStartCommand(Intent intent, int flags, int startId) {
            LogUtils.v(TAG, String.format("onStartCommand flags :%d startId :%d intent %s", flags, startId, intent));
            receiveImp(intent);
        ...
        private void dispatchOnReceiveMessage(Intent intent) {
            if(intent == null) {
                LogUtils.e(TAG , "dispatch receive message fail.");
                return ;
            }
