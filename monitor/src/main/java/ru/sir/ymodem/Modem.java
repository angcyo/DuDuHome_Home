...
class Modem {
    ...
    Logger log = LoggerFactory.getLogger("monitor.obdUpdate");
    ...
    protected boolean waitReceiverRequest(Timer timer) throws IOException {
        ...
        while (true) {
            try {
                ...
                log.debug("waitReceiverRequest  读取到字符：{}", character);
                if (character == NAK){
                    log.debug("waitReceiverRequest 收到 NAK");
                    return false;
                }

                if (character == ST_C) {
                    log.debug("waitReceiverRequest 收到 ST_C");
                    return true;
                }
     ...
     * @throws IOException
     */
    protected void send(InputStream in, boolean useBlock1K) throws IOException {
        //open file
